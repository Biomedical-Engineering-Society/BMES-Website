...
- **Language:** TypeScript
- **AI / vector DB:** Supabase (pgvector) + Groq (GPT-OSS) + LangChain
...
1. **Retrieved documents.** `scripts/ingest.ts` chunks
   [`data/source-docs/`](data/source-docs/) and stores embeddings in Supabase; a question is
   vectorised (via Hugging Face) to pull the four most relevant chunks. The only source there is
   `BMES-Knowledge-Base-v3.md`, the chapter reference the bot is grounded against. It lives outside
   `public/` on purpose: anything under `public/` is downloadable by anyone who guesses the
   filename.
...
Answers are generated by Groq. The route tries `openai/gpt-oss-120b` first and falls through to
`openai/gpt-oss-20b`, because Groq retires models without much notice and a retired model answers
404 to everything, which is what took the assistant offline in August 2026. `GROQ_MODEL` pins one.
Each model has its own token budget, so the fallthrough also doubles how many questions can be
answered per minute. To see what a key can currently reach:

```bash
curl https://api.groq.com/openai/v1/models -H "Authorization: Bearer $GROQ_API_KEY"
```

The assistant's display name is `ASSISTANT_NAME` in
