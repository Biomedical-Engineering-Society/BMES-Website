...
Question: ${query}` },
          ...
          // A model this key cannot reach answers 404. Any other failure will
          // not be fixed by swapping models, so stop and use the fallback line.
          console.error(`Generation failed on ${model}:`, llmError);
          if ((llmError as { status?: number }).status !== 404) break;
        }
      }
    }
